describe('게임 종료 테스트', () => {
  ...
  describe('게임 중단 버튼 클릭', () => {
    ...
    it('최종 자금이 표시된다', () => {
      ...
      cy.get('.final-result').should('contain', '최종 자금:');
      cy.get('.final-result').should('contain', '10,000원');
    });

    it('플레이한 라운드 수가 표시된다', () => {
      cy.get('#stop-button').click();

      cy.get('.final-result').should('contain', '플레이한 라운드:');
      cy.get('.final-result').should('contain', '0');
    });
  });
  ...
  describe('베팅 후 게임 중단', () => {
    it('베팅 후 현재 상태가 정확히 표시된다', () => {
      ...
      cy.get('.final-result').should('contain', '플레이한 라운드: 1');
    });
  });
  ...
  describe('자금 0원 시 자동 종료', () => {
    it('자금이 0원이 되면 게임이 자동으로 종료된다', () => {
      ...
      cy.get('#current-money').then(($money) => {
        ...
        if (money === 0) {
          cy.get('.final-result').should('contain', '게임 종료');
          cy.get('.final-result').should('contain', '최종 자금: 0원');
        }
      });
    });
  });
